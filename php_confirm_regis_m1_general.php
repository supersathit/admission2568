...
<?php
    ...
    $sql_chk_id = "SELECT code_id FROM regis_m1_general WHERE code_id = $code_id";
    $result_chk_id = mysqli_query($conn,$sql_chk_id);
    $row_chk_id = mysqli_fetch_array($result_chk_id);

    if($row_chk_id == ''){
        $old = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM regis_m1 WHERE code_id = $code_id"));
        ...
        $sql = "INSERT INTO `regis_m1_general` (`code_id`, `title`, `name`, `surname`, `school_name`, `school_district`, `school_province`, `grade`, `spacial`, `bdate`, `station_bdate`, `blood`, `home_id`, `home_group`, `alley`, `street`, `sub_district`, `district`, `province`, `post_code`, `tel`, `father_name`, `father_surname`, `father_occupation`, `father_tel`, `mother_name`, `mother_surname`, `mother_occupation`, `mother_tel`, `class1`, `house_regis`, `grade_file1`, `grade_file2`, `special_file`, `status`) 
        VALUES ('$code_id', '$title', '$name', '$surname', '$school_name', '$school_district', '$school_province', '$grade', '$spacial', '$bdate', '$station_bdate', '$blood', '$home_id', '$home_group', '$alley', '$street', '$sub_district', '$district', '$province', '$post_code', '$tel', '$father_name', '$father_surname', '$father_occupation', '$father_tel', '$mother_name', '$mother_surname', '$mother_occupation', '$mother_tel', '$class1', '$house_name_file', '$grade1_name_file', '$grade2_name_file', '$special_name_file', '$status');";
